/*
  I solved this by mixed of my brain and the program. I tried to adjust the parameters(startX Y, end X Y) to find the answer.
  code is totally a mess

  這題我也是靠程式跟手動混合隨意亂解的，不值得參考
*/
...
for(let y=startY; y<endY; y++) {
  ...
  for(let x=startX; x<endX; x++) {
      ...
      if (
        map[y][x] === 1 && map[y][x+99] === 1 &&
        map[y+99][x] === 1 && map[y+99][x+99] === 1
      ) {
...
for(let i=startX; i<endX; i++) {
  if (map[startY+99][i] === 1) {
    console.log('firstX', i)
    break
  }
}