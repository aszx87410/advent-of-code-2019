...
while(true) {
  let idleCount = 0
  for(let i=0; i<50; i++) {
    states[i].inputIndex = 0
    let options = states[i]
    let c = new IntCodeComputer(options)
    c.readCommand(options.state)
    let input = []
    if (!queues[i].length) {
      input = [-1]
    } else {
      let p = queues[i].shift()
      input = [p.x, p.y]
    }
    c.readInput(input)
    let result = c.run()
    states[i] = result
    if (result.waitInput && input[0] === -1) {
      idleCount++
    }
    if (result.hasOutput) {
      // console.log(i, 'output', result.outputs)
  ...
  if (idleCount === 50) {
    if (lastlast && last && last.x === lastlast.x && last.y === lastlast.y) {
